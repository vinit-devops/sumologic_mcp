...
from .config import DashboardConfig, SearchRequest, SumoLogicConfig
...
from .models.config import CollectorConfig, MetricsRequest, SourceConfig
...
class SumoLogicAPIClient:
    ...
    async def _make_request(
        ...
        await self.metrics_collector.increment_counter(
            "api_requests_total", labels={"method": method, "operation": operation_type}
        )
    ...
    async def _handle_response_errors(
        self, response: httpx.Response, endpoint: str, method: str
    ) -> None:
    ...
    async def execute_raw_request(
        ...
        """Execute a raw HTTP request to the Sumo Logic API using server credentials.

        Args:
            method: HTTP method (GET, POST, PUT, DELETE, PATCH)
            path: API path suffix (e.g., '/v1/users')
    ...
    async def search_logs(
        ...
        try:
            from_time_formatted = TimeParser.convert_time_for_api(
        ...
        try:
            validated_params = SumoLogicAPIValidator.validate_search_params(
                search_params
            )
    ...
    async def get_search_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise SearchError(
                f"Failed to get search job status: {e.message}", job_id=job_id
            ) from e
    ...
    async def get_search_results(
        ...
        try:
            ...
            raise SearchError(
                f"Failed to get search results: {e.message}", job_id=job_id
            ) from e
    ...
    async def cancel_search_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Search job {job_id} cancellation returned status {response.status_code}"
                )
            ...
            raise SearchError(
                f"Failed to cancel search job: {e.message}", job_id=job_id
            ) from e

    # Dashboard API Methods

    async def list_dashboards(
        self, limit: int = 100, offset: int = 0, filter_query: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if offset < 0:
            raise ValidationError(
                "Offset must be non-negative", field_name="offset", field_value=offset
            )
    ...
    async def create_dashboard(
        ...
        dashboard_payload = {
            ...
            "timeRange": {
                ...
                "from": {"type": "RelativeTimeRangeBoundary", "relativeTime": "-1h"},
            },
        }
    ...
    async def update_dashboard(
        ...
        logger.info(
            ...
            extra={"title": config.title, "panel_count": len(config.panels)},
        )
    ...
    async def delete_dashboard(self, dashboard_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Dashboard {dashboard_id} deletion returned status {response.status_code}"
                )
    ...
    async def query_metrics(
        ...
        try:
            ...
            start_dt, end_dt = TimeParser.validate_time_range(
                metrics_request.from_time, metrics_request.to_time
            )

            # For metrics API, we can use relative time if it's in the original format
            # or convert to epoch milliseconds
            if (
                TimeParser._is_relative_time(metrics_request.from_time)
                or metrics_request.from_time.lower() == "now"
            ):
            ...
            if (
                TimeParser._is_relative_time(metrics_request.to_time)
                or metrics_request.to_time.lower() == "now"
            ):
            ...
            raise ValidationError(
                ...
                context={
                    "from_time": metrics_request.from_time,
                    "to_time": metrics_request.to_time,
                },
            ) from e

        # Build metrics query payload
        metrics_payload = {
            "query": [{"query": metrics_request.query, "rowId": "A"}],
            "timeRange": {
                "type": "BeginBoundedTimeRange",
                "from": {"type": from_time_type, from_time_key: from_time_value},
                "to": {"type": to_time_type, to_time_key: to_time_value},
            },
    ...
    async def list_metric_sources(
        ...
        logger.debug("Listing metric sources", extra={"limit": limit, "offset": offset})
    ...
    async def get_metric_metadata(
        self, metric_name: str, source_host: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def list_collectors(
        ...
        logger.debug("Listing collectors", extra={"limit": limit, "offset": offset})
        ...
        try:
            ...
            logger.info(
                ...
                extra={
                    ...
                    "collector_type": collector.get("collector", {}).get(
                        "collectorType"
                    ),
                },
            )
    ...
    async def create_collector(
        self, collector_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        logger.info(
            ...
            extra={"collector_type": "Hosted", "ephemeral": config.ephemeral},
        )
    ...
    async def update_collector(
        self, collector_id: Union[str, int], collector_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        update_payload = {"collector": updated_config}
    ...
    async def delete_collector(self, collector_id: Union[str, int]) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Collector {collector_id_str} deletion returned status {response.status_code}"
                )
    ...
    async def list_sources(
        self, collector_id: Union[str, int], limit: int = 100, offset: int = 0
    ) -> Dict[str, Any]:
    ...
    async def create_source(
        self, collector_id: Union[str, int], source_config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def list_monitors(
        ...
        if filter_status:
            ...
            mapped_status = status_mapping.get(
                filter_status.lower(), f'status:"{filter_status}"'
            )
        ...
        # Validate parameters against official API schema
        try:
            validated_params = SumoLogicAPIValidator.validate_monitor_params(
                monitor_params
            )
        except (ValidationError, ValueError) as e:
            raise ValidationError(
    ...
    async def _search_monitors_in_folder(
        self, folder_id: str, base_query: str, limit: int, offset: int
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Validate parameters
            validated_folder_params = SumoLogicAPIValidator.validate_monitor_params(
                folder_params
            )
    ...
    async def create_monitor(self, monitor_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        monitor_payload = {
            ...
            "contentType": "Monitor",
            "queries": [{"rowId": "A", "query": config.query}],
            "triggers": [],
            "notifications": [],
        }
        ...
        for notification in config.notifications:
            notification_payload = {
                "notification": {"actionType": notification.action_type.value}
            }
    ...
    async def update_monitor(
        self, monitor_id: str, monitor_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                ...
                extra={"monitor_id": monitor_id, "name": updated_monitor.get("name")},
            )
    ...
    async def delete_monitor(self, monitor_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Monitor {monitor_id} deletion returned status {response.status_code}"
                )
                return {
                    "success": False,
    ...
    async def get_monitor_status(
        ...
        logger.info("Retrieved monitor status", extra={"status_count": len(records)})
    ...
    async def get_active_alerts(
        ...
        if self.is_circuit_breaker_open():
            logger.warning(
                ...
                extra={"severity": severity, "limit": limit, "offset": offset},
            )
    ...
    async def _get_active_alerts_with_fallback(
        self, severity: Optional[str], limit: int, offset: int
    ) -> Dict[str, Any]:
        ...
        endpoint_configs = self._get_ordered_endpoint_configs(
            severity, limit, offset, cached_config
        )
        ...
        for i, config in enumerate(endpoint_configs):
            ...
            try:
                ...
                await self._update_endpoint_failure_metrics(
                    config["name"], e.status_code
                )
    ...
    async def validate_monitor_query(
        self, query: str, monitor_type: str = "MonitorsLibraryMonitor"
    ) -> Dict[str, Any]:
        ...
        if not query or not query.strip():
            raise ValidationError(
                "Query cannot be empty", field_name="query", field_value=query
            )
        ...
        try:
            ...
            if e.status_code == 400 and any(
                ...
                for keyword in ["syntax", "parse", "invalid query", "malformed"]
            ):
    ...
    async def get_monitor_history(
        ...
        try:
            ...
            params = {"from": from_timestamp, "to": to_timestamp, "limit": limit}
    ...
    def _get_ordered_endpoint_configs(
        ...
        base_configs = [
            {
                ...
                "params": self._build_search_params(
                    "Critical", severity, limit, offset
                ),
            ...
            },
            {
                ...
                "endpoint": "/api/v1/monitors/search",
                "params": self._build_search_params(
                    "AllTriggered", severity, limit, offset
                ),
        ...
        if cached_config and self._is_cache_valid(cached_config):
            ...
            cached_config_updated = {
                **cached_config,
                "params": self._update_params_for_request(
                    cached_config, severity, limit, offset
                ),
                "is_cached": True,
                "priority": "cached",
            }
    ...
    def _build_search_params(
        self, monitor_status: str, severity: Optional[str], limit: int, offset: int
    ) -> Dict[str, Any]:
        ...
        params = {"limit": limit, "offset": offset, "query": " AND ".join(query_parts)}
    ...
    def _build_alerts_params(
        self, status: Optional[str], severity: Optional[str], limit: int, offset: int
    ) -> Dict[str, Any]:
    ...
    def _update_params_for_request(
        self, config: Dict[str, Any], severity: Optional[str], limit: int, offset: int
    ) -> Dict[str, Any]:
    ...
    async def _process_alerts_response(
        ...
        if "search" in endpoint_name:
            ...
            if severity_filter:
                alerts = [
                    ...
                    and alert.get("severity") == severity_filter
                ]
        ...
        return {
            ...
            "metadata": {
                ...
                "filtered_client_side": severity_filter is not None
                and "search" not in endpoint_name,
            },
        }

    async def _extract_alerts_from_monitors(
        self, monitors: List[Dict[str, Any]], severity_filter: Optional[str]
    ) -> List[Dict[str, Any]]:
        ...
        for monitor in monitors:
            ...
            if not isinstance(monitor, dict):
                logger.warning(
                    f"Skipping non-dictionary monitor object: {type(monitor)}"
                )
                continue

            # Check if monitor is in a triggered/alert state
            monitor_status = monitor.get("status", "").lower()
            monitor_state = monitor.get(
                "monitorStatus", monitor.get("state", "")
            ).lower()
            ...
            has_alert = (
                monitor_status in ["critical", "warning", "missingdata", "alltriggered"]
                or monitor_state
                in ["critical", "warning", "missingdata", "alltriggered"]
                or monitor.get("alertStatus", "").lower()
                in ["critical", "warning", "missingdata", "alltriggered"]
            )
    ...
    async def _get_cached_endpoint_config(
        self, operation: str
    ...
    async def _cache_successful_endpoint_config(self, config: Dict[str, Any]) -> None:
        ...
        logger.info(
            ...
            extra={
                ...
                "reliability_score": cached_config["performance_metrics"][
                    "reliability_score"
                ],
            },
        )
    ...
    async def _update_endpoint_failure_metrics(
        self, endpoint_name: str, status_code: Optional[int]
    ) -> None:
        ...
        if status_code:
            ...
            metrics["failure_codes"][str(status_code)] = (
                metrics["failure_codes"].get(str(status_code), 0) + 1
            )
    ...
    def _should_provide_empty_response(
        self, attempted_endpoints: List[Dict[str, Any]]
    ) -> bool:
        ...
        client_errors = [
            ep
            for ep in attempted_endpoints
            if ep.get("status_code") is not None
            and ep.get("status_code", 0) >= 400
            and ep.get("status_code", 0) < 500
        ]
    ...
    def _calculate_reliability_score(self, endpoint_name: str) -> float:
        ...
        """
        if (
            not hasattr(self, "_endpoint_metrics")
            or endpoint_name not in self._endpoint_metrics
        ):
        ...
        if hasattr(self, "_endpoint_cache"):
            for operation, config in self._endpoint_cache.items():
                cache_info[operation] = {
                    ...
                    "reliability_score": config.get("performance_metrics", {}).get(
