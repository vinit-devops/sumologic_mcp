...
class SumoLogicConfig(BaseModel):
    ...
    access_id: str = Field(..., description="Sumo Logic Access ID")
    access_key: str = Field(..., description="Sumo Logic Access Key")
    ...
    rate_limit_delay: float = Field(
        default=1.0, description="Delay between rate-limited requests in seconds"
    )
    ...
    def validate_endpoint(cls, v: str) -> str:
        ...
        # Clean any trailing paths (like /api, /api/v1, /api/v2) and slashes
        clean_v = v.rstrip("/")
        for suffix in ["/api/v1", "/api/v2", "/api"]:
            if clean_v.endswith(suffix):
                clean_v = clean_v[: -len(suffix)].rstrip("/")

        if not clean_v.endswith(".sumologic.com"):
    ...
    def from_env(cls) -> "SumoLogicConfig":
        ...
        def get_env_with_fallback(
            primary: str, fallback: str, default: str = ""
        ) -> str:
        ...
        return cls(
            ...
            max_retries=int(
                get_env_with_fallback("SUMOLOGIC_MAX_RETRIES", "MAX_RETRIES", "3")
            ),
            rate_limit_delay=float(
                get_env_with_fallback(
                    "SUMOLOGIC_RATE_LIMIT_DELAY", "RATE_LIMIT_DELAY", "1.0"
                )
            ),
            log_level=get_env_with_fallback("SUMOLOGIC_LOG_LEVEL", "LOG_LEVEL", "INFO"),
            log_format=get_env_with_fallback(
                "SUMOLOGIC_LOG_FORMAT", "LOG_FORMAT", "json"
            ),
            server_name=get_env_with_fallback(
                "SUMOLOGIC_SERVER_NAME", "SERVER_NAME", "sumologic-mcp-server"
            ),
            server_version=get_env_with_fallback(
                "SUMOLOGIC_SERVER_VERSION", "SERVER_VERSION", "0.1.0"
            ),
            # Reference compatibility fields
            query_timeout=int(
                get_env_with_fallback("QUERY_TIMEOUT", "SUMOLOGIC_QUERY_TIMEOUT", "300")
            ),
            max_results=int(
                get_env_with_fallback("MAX_RESULTS", "SUMOLOGIC_MAX_RESULTS", "1000")
            ),
            default_vmware_source=get_env_with_fallback(
                "DEFAULT_VMWARE_SOURCE",
                "SUMOLOGIC_DEFAULT_VMWARE_SOURCE",
                "otel/vmware",
            ),
        )
        ...
        if config_path:
            ...
            try:
                ...
                if not isinstance(file_config, dict):
                    raise ValueError(
                        f"Configuration file must contain a JSON object, got {type(file_config).__name__}"
                    )
    ...
    def validate_required_fields(self) -> Dict[str, str]:
        ...
        if not self.access_id:
            errors["access_id"] = (
                "Sumo Logic Access ID is required. Set SUMOLOGIC_ACCESS_ID environment variable or provide in config file."
            )

        if not self.access_key:
            errors["access_key"] = (
                "Sumo Logic Access Key is required. Set SUMOLOGIC_ACCESS_KEY environment variable or provide in config file."
            )

        if not self.endpoint:
            errors["endpoint"] = (
                "Sumo Logic API endpoint is required. Set SUMOLOGIC_ENDPOINT environment variable or provide in config file."
            )
...
class SearchRequest(BaseModel):
    ...
    limit: int = Field(
        default=100, ge=1, le=10000, description="Maximum results to return"
    )
...
class DashboardConfig(BaseModel):
    ...
    refresh_interval: Optional[int] = Field(
        None, ge=30, description="Auto-refresh interval in seconds (minimum 30)"
    )
