...
from .exceptions import APIError, SumoLogicError, TimeoutError
...
class MetricsCollector:
    ...
    async def record_timer(
        ...
        async with self._lock:
            ...
            metric_value = MetricValue(
                value=duration_ms, timestamp=datetime.utcnow(), labels=labels or {}
            )
            metric["values"].append(metric_value)
...
class ConnectionMonitor:
    ...
    def __init__(self, check_interval: float = 60.0):
        ...
        logger.info(
            f"Initialized connection monitor with check_interval={check_interval}s"
        )
    ...
    async def register_connection(
        self, name: str, endpoint: str, check_func: Optional[Callable] = None
    ):
    ...
    async def check_connection(self, name: str) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                details={"endpoint": connection["endpoint"], "consecutive_failures": 0},
            )
    ...
    async def get_connection_status(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        async with self._lock:
            ...
            return {
                ...
                "last_failure": connection["last_failure"].isoformat()
    ...
    async def get_all_connections_status(self) -> Dict[str, Any]:
        ...
        """
        async with self._lock:
            names = list(self._connection_status.keys())
        result = {}
        for name in names:
...
class HealthChecker:
    ...
    async def register_health_check(
        ...
        """
        async with self._lock:
            self._health_checks[name] = {"func": check_func, "description": description}
        ...
        return {
            ...
            "monitoring": {
                ...
                "connection_monitor_active": (
                    self.connection_monitor._monitoring_task is not None
                    and not self.connection_monitor._monitoring_task.done()
                ),
