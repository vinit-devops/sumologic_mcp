...
from .exceptions import APIError, SumoLogicError, ValidationError
...
class SumoLogicMCPServer:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                "Failed to start MCP server", error=str(e), error_type=type(e).__name__
            )
            raise
    ...
    async def handle_tool_call(
        ...
        try:
            ...
            if tool_name not in self.tool_handlers:
                ...
                self.logger.error(
                    error_msg, available_tools=list(self.tool_handlers.keys())
                )
            ...
            return {
                "content": [
                    {"type": "text", "text": response_text, "annotations": None}
                ],
                "isError": False,
            }
            ...
            # Log error response
            self.error_handler.log_response(
                tool_name, success=False, execution_time_ms=execution_time
            )
    ...
    def register_tools(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                "Failed to register tools", error=str(e), error_type=type(e).__name__
            )
            raise
    ...
    async def _check_tool_handlers_health(self) -> None:
        ...
        for tool in required_tools:
            if tool is None:
                raise SumoLogicError(
                    f"Tool handler {type(tool).__name__} not initialized"
                )
    ...
    async def _handle_health_check(
        self, include_metrics: bool = False, include_connections: bool = True
    ) -> Dict[str, Any]:
