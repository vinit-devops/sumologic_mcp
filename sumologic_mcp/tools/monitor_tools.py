...
class MonitorTools:
    ...
    async def list_monitors(
        ...
        async def _list_monitors_impl():
                ...
                if filter_type not in valid_types:
                    raise MonitorValidationError(
                        ...
                        context=create_monitor_error_context(
                            "list_monitors", valid_types=valid_types
                        ),
                    )
            ...
            if filter_status:
                ...
                if filter_status not in valid_statuses:
                    raise MonitorValidationError(
                        ...
                        context=create_monitor_error_context(
                            "list_monitors", valid_statuses=valid_statuses
                        ),
                    )
            ...
            enhanced_monitors = self._enhance_monitors_with_folder_info(
                filtered_monitors
            )
            ...
            response = {
                ...
                "metadata": {
                    ...
                    "offset": validated_offset,
                    "limit": validated_limit,
                    "has_more": (validated_offset + len(enhanced_monitors))
                    < total_count,
                },
    ...
    def _filter_by_name_pattern(
        self, monitors: List[Dict[str, Any]], name_pattern: str
    ) -> List[Dict[str, Any]]:
        ...
        if "*" in name_pattern or "?" in name_pattern:
            ...
            try:
                ...
                logger.warning(
                    f"Invalid regex pattern '{name_pattern}', falling back to substring matching"
                )
    ...
    def _filter_by_monitor_type(
        self, monitors: List[Dict[str, Any]], monitor_type: str
    ) -> List[Dict[str, Any]]:
    ...
    def _filter_by_status(
        self, monitors: List[Dict[str, Any]], status_filter: str
    ) -> List[Dict[str, Any]]:
        ...
        if status_filter == "enabled":
            return [
                monitor for monitor in monitors if not monitor.get("isDisabled", False)
            ]
        elif status_filter == "disabled":
            return [monitor for monitor in monitors if monitor.get("isDisabled", False)]
            ...
            logger.warning(
                f"Unknown status filter '{status_filter}', returning all monitors"
            )
            return monitors

    def _calculate_folder_statistics(
        ...
        for monitor in monitors:
            ...
            if content_type == "MonitorsLibraryFolder":
                ...
                if (
                    folder_path
                    and folder_path != "/"
                    and not folder_path.startswith("/unknown")
                ):
    ...
    def _enhance_monitors_with_folder_info(
        self, monitors: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    async def search_monitors(
        ...
        async def _search_monitors_impl():
            ...
            if search_fields is not None:
                ...
                invalid_fields = [
                    field for field in search_fields if field not in valid_fields
                ]
                if invalid_fields:
                    raise MonitorValidationError(
            ...
            try:
                ...
                raise RateLimitError(
                    ...
                    limit_type=e.limit_type,
                    context=create_monitor_error_context(
                        "search_monitors", search_query=search_query.strip()
                    ),
                ) from e
            except Exception as e:
                raise MonitorError(
            ...
            response = {
                ...
                "search_metadata": {
                    ...
                    "offset": validated_offset,
                    "limit": validated_limit,
                    "has_more": (validated_offset + len(paginated_results))
                    < total_results,
                },
            }
    ...
    async def get_monitor(self, monitor_id: str) -> Dict[str, Any]:
        ...
        # Execute with comprehensive error handling
        return await self.error_handler.execute_with_error_handling(
            "get_monitor", _get_monitor_impl, monitor_id=monitor_id
        )
    ...
    def _format_monitor_details(self, monitor_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        formatted_notifications = self._format_notification_configurations(
            notifications
        )
    ...
    def sort_monitors(
        ...
        try:
            ...
            logger.debug(f"Sorted {len(monitors)} monitors by {sort_by} ({sort_order})")
    ...
    def _calculate_monitor_statistics(
        self, monitors: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _format_trigger_conditions(
    ...
    def _format_notification_configurations(
        ...
        for notification in notifications:
            ...
            formatted_notification = {
                "type": action_type,
                "friendly_type": self._convert_notification_type_to_friendly(
                    action_type
                ),
                "subject": subject,
            ...
            if action_type == "EmailAction":
                ...
                formatted_notification["destinations"] = [
                    {"type": "email", "address": recipient} for recipient in recipients
                ]
                ...
                if integration_key:
                    formatted_notification["destinations"] = [
                        {
                            "type": "pagerduty",
                            "integration_key": integration_key[:8] + "...",
                        }
                    ]
            ...
            formatted_notification["summary"] = (
                f"{formatted_notification['friendly_type']} to {dest_summary}"
            )
    ...
    def _format_threshold_description(
        self, threshold: float, threshold_type: str, occurrence_type: str
    ) -> str:
    ...
    def _extract_query_components(
        ...
        """
        components = {"keywords": [], "operators": [], "fields": [], "functions": []}
        ...
        if query_language == "sumoql":
            keywords = re.findall(
                r"\b(where|and|or|by|count|sum|avg|max|min|parse|json|csv|timeslice)\b",
                query,
                re.IGNORECASE,
            )
    ...
    def _generate_syntax_hints(self, query: str, query_language: str) -> List[str]:
        ...
        if query_language == "sumoql":
            ...
            hints.append(
                "Metrics query - supports metric(), quantize(), and aggregation functions"
            )
    ...
    def _assess_configuration_health(
        ...
        if "Critical" not in formatted_triggers:
            ...
            recommendations.append(
                "Consider adding a Critical trigger for high-severity alerts"
            )
        ...
        if not formatted_notifications:
            ...
            recommendations.append(
                "Add at least one notification action to receive alerts"
            )
        ...
        if not description or len(description.strip()) < 10:
            ...
            recommendations.append(
                "Add a detailed description explaining the monitor's purpose"
            )
        ...
        if evaluation_delay == "0m":
            recommendations.append(
                "Consider adding evaluation delay to reduce false positives"
            )
        ...
        async def _create_monitor_impl():
            ...
            if len(name.strip()) > 255:
                raise MonitorValidationError(
                    ...
                    field_value=name,
                    context=create_monitor_error_context(
                        "create_monitor", name_length=len(name)
                    ),
                )
            ...
            if not query or not query.strip():
                raise MonitorValidationError(
                    ...
                    field_value=query,
                    context=create_monitor_error_context(
                        "create_monitor", monitor_name=name.strip()
                    ),
                )
            ...
            try:
                created_monitor = await self.api_client.create_monitor(
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timeout_configuration": {
                "list_monitors": self.error_handler.timeout_manager.get_timeout(
                    "list_monitors"
                ),
                "get_monitor": self.error_handler.timeout_manager.get_timeout(
                    "get_monitor"
                ),
                "create_monitor": self.error_handler.timeout_manager.get_timeout(
                    "create_monitor"
                ),
                "update_monitor": self.error_handler.timeout_manager.get_timeout(
                    "update_monitor"
                ),
                "delete_monitor": self.error_handler.timeout_manager.get_timeout(
                    "delete_monitor"
                ),
            },
        }
    ...
    async def update_monitor(
        ...
        **kwargs,
        ...
        async def _update_monitor_impl():
            # Validate monitor_id with enhanced validation
            validated_monitor_id = await validate_monitor_id(
                monitor_id, "update_monitor"
            )
        ...
        try:
            ...
            if not version_check_result["can_update"]:
                raise APIError(
                    ...
                    context={
                        ...
                        "conflict_type": "version_mismatch",
                        "current_version": version_check_result.get("current_version"),
                        "expected_version": version_check_result.get(
                            "expected_version"
                        ),
                    },
                )
            ...
            logger.info(
                ...
                extra={
                    ...
                    "changed_fields": response.get("changes", {}).get(
                        "modified_fields", []
                    ),
            ...
            raise APIError(
                ...
                context={"operation": "update_monitor", "monitor_id": monitor_id},
            ) from e
        ...
        try:
            ...
            if "trigger_conditions" in changed_fields:
                ...
                validation_result["warnings"].extend(
                    trigger_validation.get("warnings", [])
                )
            ...
            if "notifications" in changed_fields:
                ...
                validation_result["errors"].extend(
                    notification_validation.get("errors", [])
                )
                validation_result["warnings"].extend(
                    notification_validation.get("warnings", [])
                )
            ...
            if "evaluation_delay" in changed_fields:
                ...
                validation_result["errors"].extend(
                    schedule_validation.get("errors", [])
                )
                validation_result["warnings"].extend(
                    schedule_validation.get("warnings", [])
                )
    ...
    async def _validate_updated_trigger_conditions(
        self, new_conditions: Dict[str, Any], current_conditions: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Validate updated trigger conditions and thresholds.
        ...
        try:
            ...
            for trigger_type, condition in new_conditions.items():
                ...
                if current_condition:
                    ...
                    if (
                        current_threshold
                        and threshold
                        and abs(threshold - current_threshold)
            ...
            validation_result["errors"].append(
                f"Error validating trigger conditions: {str(e)}"
            )
            return validation_result
        ...
        try:
            ...
            for i, notification in enumerate(new_notifications):
                if not isinstance(notification, dict):
                    validation_result["errors"].append(
                        f"Notification {i} must be a dictionary"
                    )
                    continue
                ...
                if action_type == "EmailAction":
                    ...
                    if not recipients:
                        validation_result["errors"].append(
                            f"Email notification {i} missing recipients"
                        )
                    elif not isinstance(recipients, list):
                        validation_result["errors"].append(
                            f"Email notification {i} recipients must be a list"
                        )
                    else:
                    ...
                    if not webhook_url:
                        validation_result["errors"].append(
                            f"Webhook notification {i} missing webhook_url"
                        )
            ...
            validation_result["errors"].append(
                f"Error validating notifications: {str(e)}"
            )
            return validation_result

    async def _validate_schedule_modification(
        self, new_delay: Optional[str], current_delay: Optional[str]
    ) -> Dict[str, Any]:
        """Add schedule modification with validation.
        ...
        try:
            ...
            validation_result["errors"].append(
                f"Error validating schedule modification: {str(e)}"
            )
            return validation_result

    def _validate_monitor_name_update(
        self, new_name: Optional[str], current_name: Optional[str]
    ) -> Dict[str, Any]:
        ...
        try:
            if new_name is not None:
                ...
                if not new_name.strip():
                    ...
                    if re.search(r'[<>:"/\\|?*]', new_name):
                        validation_result["errors"].append(
                            "Monitor name contains invalid characters"
                        )
            ...
            validation_result["errors"].append(
                f"Error validating name update: {str(e)}"
            )
            return validation_result
    ...
    async def _handle_version_conflicts(
        ...
        try:
            ...
            return {"can_update": True, "version_check_failed": True, "error": str(e)}
    ...
    def _generate_update_recommendations(
        self, changes: Dict[str, Any], validation_results: Dict[str, Any]
    ) -> List[str]:
        ...
        if "trigger_conditions" in changed_fields:
            recommendations.append(
                "Test the updated trigger conditions to ensure they work as expected"
            )

        if "notifications" in changed_fields:
            recommendations.append(
                "Verify notification destinations are working correctly"
            )

        if "query" in changed_fields:
            recommendations.append(
                "Monitor the updated query performance and result accuracy"
            )
        ...
        if "is_disabled" in changed_fields:
            ...
            if disabled_change.get("to") is True:
                recommendations.append(
                    "Monitor is now disabled - enable when ready to resume monitoring"
                )
            elif disabled_change.get("to") is False:
                recommendations.append(
                    "Monitor is now enabled and will start evaluating immediately"
                )
        ...
        if warnings:
            recommendations.append(
                f"Review {len(warnings)} configuration warnings for optimization opportunities"
            )

        # General recommendations
        if len(changed_fields) > 3:
            recommendations.append(
                "Consider testing the monitor thoroughly due to multiple configuration changes"
            )

        recommendations.append(
            "Check monitor status and alert history to verify the update is working correctly"
        )
    ...
    async def _perform_comprehensive_validation(
        ...
        notification_validation = await self._validate_notification_configurations(
            notifications
        )
        ...
        cross_validation = self._perform_cross_validation_checks(
            config, trigger_conditions, notifications
        )
    ...
    async def _validate_query_syntax(
        self, query: str, monitor_type: str
    ) -> Dict[str, Any]:
        ...
        if len(query) > 10000:  # Reasonable limit for query length
            ...
            validation_result["warnings"].append(
                "Query is very long, consider simplifying for better performance"
            )
    ...
    def _validate_sumoql_syntax(self, query: str) -> Dict[str, Any]:
        ...
        if not any(
            ...
            validation_result["warnings"].append(
                "Query may be too restrictive - consider adding source filters"
            )

        # Check for potentially expensive operations
        if "parse regex" in query_lower and "where" not in query_lower:
            validation_result["warnings"].append(
                "Regex parsing without WHERE clause may be expensive"
            )

        # Check for proper aggregation in monitor queries
        if not any(
            agg in query_lower
            for agg in ["count", "sum", "avg", "max", "min", "timeslice"]
        ):
            validation_result["warnings"].append(
                "Monitor queries typically need aggregation functions"
            )
    ...
    def _validate_metrics_query_syntax(self, query: str) -> Dict[str, Any]:
        ...
        if "metric=" not in query_lower and "metric(" not in query_lower:
            validation_result["errors"].append(
                "Metrics queries must include metric specification"
            )

        # Check for proper aggregation
        if not any(
            agg in query_lower
            for agg in ["avg", "sum", "max", "min", "count", "quantize"]
        ):
            validation_result["warnings"].append(
                "Metrics queries typically need aggregation functions"
            )

        # Check for time quantization
        if "quantize" not in query_lower:
            validation_result["warnings"].append(
                "Consider using quantize() for better performance in metrics queries"
            )
    ...
    def _validate_sli_query_syntax(self, query: str) -> Dict[str, Any]:
        ...
        if "success" not in query_lower and "error" not in query_lower:
            validation_result["warnings"].append(
                "SLI queries typically define success or error conditions"
            )

        # Check for ratio calculations
        if "/" not in query and "ratio" not in query_lower:
            validation_result["warnings"].append(
                "SLI queries typically calculate success/total ratios"
            )
    ...
    def _validate_trigger_conditions(
        ...
        for trigger_type, condition in trigger_conditions.items():
            ...
            condition_validation = self._validate_single_trigger_condition(
                trigger_type, condition
            )
            validation_result["errors"].extend(condition_validation.get("errors", []))
            validation_result["warnings"].extend(
                condition_validation.get("warnings", [])
            )
            if condition_validation.get("errors"):
                validation_result["valid"] = False

        # Cross-validate trigger conditions
        cross_trigger_validation = self._cross_validate_trigger_conditions(
            trigger_conditions
        )
        validation_result["warnings"].extend(
            cross_trigger_validation.get("warnings", [])
        )

        # Check for recommended trigger configurations
        if "Critical" not in trigger_conditions:
            validation_result["warnings"].append(
                "Consider adding a Critical trigger condition for high-severity alerts"
            )

        return validation_result

    def _validate_single_trigger_condition(
        self, trigger_type: str, condition: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for field in required_fields:
            if field not in condition:
                validation_result["errors"].append(
                    f"{trigger_type} trigger missing required field: {field}"
                )
        ...
        if "threshold" in condition:
            ...
            if not isinstance(threshold, (int, float)):
                validation_result["errors"].append(
                    f"{trigger_type} trigger threshold must be a number"
                )
            elif threshold < 0:
                validation_result["warnings"].append(
                    f"{trigger_type} trigger has negative threshold, ensure this is intended"
                )
        ...
        if "time_range" in condition:
            ...
            if not self._validate_time_range_format(time_range):
                validation_result["errors"].append(
                    f"{trigger_type} trigger has invalid time_range format: {time_range}"
                )
    ...
    def _cross_validate_trigger_conditions(
        self, trigger_conditions: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if "Critical" in trigger_conditions and "Warning" in trigger_conditions:
            ...
            critical_type = trigger_conditions["Critical"].get(
                "threshold_type", "GreaterThan"
            )
            warning_type = trigger_conditions["Warning"].get(
                "threshold_type", "GreaterThan"
            )
            ...
            if (
                ...
                and critical_type == warning_type
            ):
    ...
    async def _validate_notification_configurations(
        self, notifications: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if not notifications:
            validation_result["warnings"].append(
                "No notification actions configured - alerts will not be sent"
            )
            return validation_result

        # Validate each notification configuration
        for i, notification in enumerate(notifications):
            notification_validation = await self._validate_single_notification(
                i, notification
            )
            validation_result["errors"].extend(
                notification_validation.get("errors", [])
        ...
        cross_notification_validation = self._cross_validate_notifications(
            notifications
        )
        validation_result["warnings"].extend(
            cross_notification_validation.get("warnings", [])
        )

        return validation_result

    async def _validate_single_notification(
        self, index: int, notification: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if "action_type" not in notification:
            validation_result["errors"].append(
                f"Notification {index + 1}: Missing required 'action_type' field"
            )
            return validation_result
        ...
        if action_type == "EmailAction":
            email_validation = await self._validate_email_notification(
                index, notification
            )
            ...
            webhook_validation = await self._validate_webhook_notification(
                index, notification
            )
            ...
            slack_validation = await self._validate_slack_notification(
                index, notification
            )
            ...
            pagerduty_validation = await self._validate_pagerduty_notification(
                index, notification
            )
            validation_result["errors"].extend(pagerduty_validation.get("errors", []))
            validation_result["warnings"].extend(
                pagerduty_validation.get("warnings", [])
            )

        # Validate common notification fields
        common_validation = self._validate_common_notification_fields(
            index, notification
        )
    ...
    async def _validate_email_notification(
        self, index: int, notification: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not recipients:
            validation_result["errors"].append(
                f"Email notification {index + 1}: No recipients specified"
            )
            return validation_result

        if not isinstance(recipients, list):
            validation_result["errors"].append(
                f"Email notification {index + 1}: Recipients must be a list"
            )
            return validation_result
        ...
        for i, email in enumerate(recipients):
            if not isinstance(email, str):
                validation_result["errors"].append(
                    f"Email notification {index + 1}: Recipient {i + 1} must be a string"
                )
                continue

            if not re.match(email_pattern, email.strip()):
                validation_result["errors"].append(
                    f"Email notification {index + 1}: Invalid email address '{email}'"
                )

        # Check for reasonable number of recipients
        if len(recipients) > 50:
            validation_result["warnings"].append(
                f"Email notification {index + 1}: Large number of recipients ({len(recipients)}) may cause delivery issues"
            )

        return validation_result

    async def _validate_webhook_notification(
        self, index: int, notification: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not webhook_url:
            validation_result["errors"].append(
                f"Webhook notification {index + 1}: Missing required 'webhook_url' field"
            )
            return validation_result

        if not isinstance(webhook_url, str):
            validation_result["errors"].append(
                f"Webhook notification {index + 1}: webhook_url must be a string"
            )
            return validation_result
        ...
        if not webhook_url.startswith(("http://", "https://")):
            validation_result["errors"].append(
                f"Webhook notification {index + 1}: webhook_url must start with http:// or https://"
            )

        # Recommend HTTPS for security
        if webhook_url.startswith("http://"):
            validation_result["warnings"].append(
                f"Webhook notification {index + 1}: Consider using HTTPS for better security"
            )
        ...
        try:
            ...
            if not parsed.netloc:
                validation_result["errors"].append(
                    f"Webhook notification {index + 1}: Invalid webhook URL format"
                )
        except Exception:
            validation_result["errors"].append(
                f"Webhook notification {index + 1}: Invalid webhook URL format"
    ...
    async def _validate_slack_notification(
        self, index: int, notification: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not webhook_url and not channel:
            validation_result["errors"].append(
                f"Slack notification {index + 1}: Must specify either 'webhook_url' or 'channel'"
            )
            return validation_result
        ...
        if webhook_url:
            if not isinstance(webhook_url, str):
                validation_result["errors"].append(
                    f"Slack notification {index + 1}: webhook_url must be a string"
                )
            elif not webhook_url.strip().startswith("https://hooks.slack.com/"):
                validation_result["warnings"].append(
                    f"Slack notification {index + 1}: webhook_url should be a Slack webhook URL"
                )
        ...
        if channel:
            if not isinstance(channel, str):
                validation_result["errors"].append(
                    f"Slack notification {index + 1}: channel must be a string"
                )
            else:
                channel = channel.strip()
                if not channel.startswith("#") and not channel.startswith("@"):
                    validation_result["warnings"].append(
                        f"Slack notification {index + 1}: channel should start with # for channels or @ for users"
                    )

        return validation_result

    async def _validate_pagerduty_notification(
        self, index: int, notification: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not integration_key:
            validation_result["errors"].append(
                f"PagerDuty notification {index + 1}: Missing required 'integration_key' field"
            )
            return validation_result

        if not isinstance(integration_key, str):
            validation_result["errors"].append(
                f"PagerDuty notification {index + 1}: integration_key must be a string"
            )
            return validation_result
        ...
        if len(integration_key) != 32:
            validation_result["warnings"].append(
                f"PagerDuty notification {index + 1}: integration_key should be 32 characters long"
            )

        # Check for valid characters (alphanumeric)
        if not integration_key.replace("-", "").replace("_", "").isalnum():
            validation_result["warnings"].append(
                f"PagerDuty notification {index + 1}: integration_key contains unexpected characters"
            )

        return validation_result

    def _validate_common_notification_fields(
        self, index: int, notification: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if subject is not None:
            if not isinstance(subject, str):
                validation_result["errors"].append(
                    f"Notification {index + 1}: subject must be a string"
                )
            elif len(subject.strip()) == 0:
                validation_result["warnings"].append(
                    f"Notification {index + 1}: subject is empty"
                )
            elif len(subject) > 255:
                validation_result["errors"].append(
                    f"Notification {index + 1}: subject is too long (maximum 255 characters)"
                )
        ...
        if message_body is not None:
            if not isinstance(message_body, str):
                validation_result["errors"].append(
                    f"Notification {index + 1}: message_body must be a string"
                )
            elif len(message_body) > 2000:
                validation_result["errors"].append(
                    f"Notification {index + 1}: message_body is too long (maximum 2000 characters)"
                )
            elif len(message_body.strip()) == 0:
                validation_result["warnings"].append(
                    f"Notification {index + 1}: message_body is empty"
                )

        return validation_result

    def _cross_validate_notifications(
        self, notifications: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
            ...
            if action_type == "EmailAction":
                ...
                for recipient in recipients:
                    ...
                    if dest_id in seen_destinations:
                        validation_result["warnings"].append(
                            f"Duplicate email recipient '{recipient}' found in multiple notifications"
                        )
                ...
                if webhook_url:
                    ...
                    if dest_id in seen_destinations:
                        validation_result["warnings"].append(
                            f"Duplicate webhook URL found in multiple notifications"
                        )
                    seen_destinations.add(dest_id)
                ...
                if webhook_url:
                    ...
                    if dest_id in seen_destinations:
                        validation_result["warnings"].append(
                            f"Duplicate Slack webhook URL found in multiple notifications"
                        )
                    seen_destinations.add(dest_id)
                if channel:
                    ...
                    if dest_id in seen_destinations:
                        validation_result["warnings"].append(
                            f"Duplicate Slack channel '{channel}' found in multiple notifications"
                        )
                    seen_destinations.add(dest_id)
                ...
                if integration_key:
                    ...
                    if dest_id in seen_destinations:
                        validation_result["warnings"].append(
                            f"Duplicate PagerDuty integration key found in multiple notifications"
                        )
                    seen_destinations.add(dest_id)
        ...
        if len(notifications) > 3 and len(unique_types) == 1:
            validation_result["warnings"].append(
                "Consider using different notification types for better redundancy"
            )
    ...
    def _perform_cross_validation_checks(
        ...
        if "Critical" in trigger_conditions and notifications:
            ...
            has_immediate = any(
                notif.get("action_type") in immediate_types for notif in notifications
            )
            if not has_immediate:
                validation_result["warnings"].append(
                    "Critical triggers should have immediate notification types (PagerDuty, Slack)"
                )
        ...
        if monitor_type == "MetricsMonitor" and "metric" not in query_lower:
            validation_result["warnings"].append(
                "Metrics monitor should use metrics queries with metric() function"
            )
        elif monitor_type == "MonitorsLibraryMonitor" and "metric" in query_lower:
            validation_result["warnings"].append(
                "Log monitor should not use metrics queries"
            )
    ...
    def _format_monitor_creation_response(
        self, created_monitor: Dict[str, Any], validation_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "monitor": {
                ...
                "type": self._convert_monitor_type_to_friendly(
                    created_monitor.get("monitorType")
                ),
                "api_type": created_monitor.get("monitorType"),
                "status": (
                    "disabled" if created_monitor.get("isDisabled") else "enabled"
                ),
            ...
            "validation": {
                ...
                "trigger_conditions_valid": validation_results.get(
                    "trigger_conditions_valid"
                ),
    ...
    def _generate_next_steps_recommendations(
        ...
        if created_monitor.get("isDisabled"):
            ...
            recommendations.append(
                "Monitor is active and will start evaluating immediately"
            )
        ...
        if not created_monitor.get("notifications"):
            recommendations.append(
                "Add notification actions to receive alerts when triggers fire"
            )
        ...
        recommendations.append(
            "Test the monitor by checking its status and alert history"
        )
        recommendations.append(
            "Document the monitor's purpose and expected behavior for your team"
        )
    ...
    async def delete_monitor(self, monitor_id: str) -> Dict[str, Any]:
        ...
        async def _delete_monitor_impl():
            ...
            try:
                ...
                monitor_info = monitor_info_response.get(
                    "monitor", monitor_info_response
                )
            ...
            try:
                deletion_result = await self.api_client.delete_monitor(
                    validated_monitor_id
                )
            except APIError as e:
                # The error handler will convert this to appropriate monitor error
                raise e
        ...
        try:
            # Step 1: Retrieve monitor information before deletion for audit trail
            logger.debug(
                f"Retrieving monitor information before deletion: {monitor_id}"
            )
                ...
                logger.info(
                    ...
                    extra={
                        ...
                        "notification_count": len(
                            monitor_info.get("notifications", [])
                        ),
                    },
                )
            ...
            response = {
                ...
                "audit_trail": {
                    ...
                    "pre_deletion_status": (
                        "enabled"
                        if not monitor_info.get("isDisabled", False)
                        else "disabled"
                    ),
            ...
            logger.info(
                ...
                extra={
                    ...
                    "deletion_timestamp": response["deleted_monitor"][
                        "deletion_timestamp"
                    ],
                },
            )
    ...
    async def _handle_cascade_deletion_info(
        self, monitor_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            cascade_info["removed_items"].append(
                "All historical execution data and alert history"
            )
            cascade_info["removed_items"].append(
                "All associated alert instances and status history"
            )
    ...
    async def get_monitor_status(
        self, monitor_id: Optional[str] = None, filter_status: Optional[str] = None
    ) -> Dict[str, Any]:
            ...
            if filter_status:
                ...
                if filter_status not in valid_statuses:
                    raise MonitorValidationError(
                        ...
                        field_value=filter_status,
                        context=create_monitor_error_context(
                            "get_monitor_status", valid_statuses=valid_statuses
                        ),
                    )
            ...
            if validated_monitor_id:
                ...
                try:
                    ...
                    formatted_status = self._format_single_monitor_status(
                        status_data, validated_monitor_id
                    )
                ...
                try:
                    ...
                    if filter_status:
                        status_list = self._filter_status_by_condition(
                    ...
                    try:
                        status_stats = self._calculate_status_statistics(
                            formatted_statuses
                        )
                    except Exception as e:
                        logger.warning(
    ...
    def _format_single_monitor_status(
        self, status_data: Dict[str, Any], monitor_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            formatted_status = {
                ...
                "status_description": self._get_status_description(
                    normalized_status, current_trigger_severity
                ),
                ...
                "last_evaluation": self._format_timestamp_with_relative(
                    last_evaluation
                ),
                "next_evaluation": self._format_timestamp_with_relative(
                    next_evaluation
                ),
            ...
            if (
                normalized_status == MonitorStatus.TRIGGERED
                and current_trigger_severity
            ):
                formatted_status["trigger_details"] = self._extract_trigger_details(
                    status_data
                )
    ...
    def _filter_status_by_condition(
        self, status_list: List[Dict[str, Any]], filter_status: str
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_status_statistics(
        self, formatted_statuses: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "alert_summary": {
                ...
                "average_triggers_per_monitor": round(
                    total_triggers_24h / total_monitors, 2
                ),
                "severity_breakdown": severity_counts,
            },
            "health_score": self._calculate_overall_health_score(
                status_counts, total_monitors
            ),
        }

    def _get_status_description(
        self, status: MonitorStatus, severity: Optional[str] = None
    ) -> str:
    ...
    def _format_timestamp_with_relative(
        ...
        try:
            ...
            if status == MonitorStatus.TRIGGERED:
                indicators["recommendations"].append(
                    "Investigate current alert and resolve underlying issue"
                )

            if trigger_count_24h > 5:
                indicators["recommendations"].append(
                    "Consider adjusting trigger thresholds to reduce alert noise"
                )

            if status == MonitorStatus.DISABLED:
                indicators["recommendations"].append(
                    "Review if monitor should be re-enabled"
                )

            if trigger_count_24h == 0 and last_triggered:
                indicators["recommendations"].append(
                    "Monitor appears stable - consider if thresholds are appropriate"
                )
        ...
        try:
            ...
            if trigger_details["triggered_at"]:
                try:
                    ...
                    triggered_dt = dateutil.parser.parse(
                        trigger_details["triggered_at"]
                    )
                    now = datetime.now(timezone.utc)
    ...
    def _calculate_overall_health_score(
        self, status_counts: Dict[str, int], total_monitors: int
    ) -> Dict[str, Any]:
        ...
        if total_monitors == 0:
            return {
                "score": 0,
                "grade": "N/A",
                "description": "No monitors to evaluate",
            }
    ...
    async def get_active_alerts(
        ...
        async def _get_active_alerts_impl():
            ...
            if severity:
                ...
                if severity not in valid_severities:
                    raise MonitorValidationError(
                        ...
                        context=create_monitor_error_context(
                            "get_active_alerts", valid_severities=valid_severities
                        ),
                    )
            ...
            try:
                ...
                raise RateLimitError(
                    ...
                    limit_type=e.limit_type,
                    context=create_monitor_error_context(
                        "get_active_alerts", severity=severity, limit=limit
                    ),
                ) from e
        ...
        logger.info(
            "Getting active alerts", extra={"severity": severity, "limit": limit}
        )
        ...
        try:
            ...
            formatted_alerts = [
                self._format_single_alert(alert_data) for alert_data in alerts_data
            ]
            ...
            logger.error(
                ...
                extra={"error": str(e), "severity": severity, "limit": limit},
            )
            ...
            if e.status_code == 403:
                raise APIError(
                    ...
                    context={"operation": "get_active_alerts", "severity": severity},
                ) from e
            else:
                raise APIError(
    ...
    def _format_no_alerts_response(
        self, severity: Optional[str], total_count: int
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "recommendations": [
                    (
                        "All monitors are operating normally"
                        if not severity
                        else f"No {severity} alerts currently active"
                    )
                ],
            },
        }
        ...
        try:
            ...
            formatted_alert = {
                ...
                "trigger_details": {
                    "value": trigger_value,
                    "threshold": threshold,
                    "comparison": self._format_threshold_comparison(
                        trigger_value, threshold
                    ),
                    "query": query[:200] + "..." if len(query) > 200 else query,
                },
                "alert_context": self._generate_alert_context(alert_data),
                "urgency_indicators": self._generate_urgency_indicators(
                    severity, triggered_at, trigger_value, threshold
                ),
            }

            # Add duration information
            if triggered_at:
                formatted_alert["duration"] = self._calculate_alert_duration(
                    triggered_at
                )
    ...
    def _get_severity_priority(self, severity: str) -> int:
        ...
        """
        priority_map = {"Critical": 1, "Warning": 2, "MissingData": 3}
        return priority_map.get(severity, 999)

    def _format_threshold_comparison(
        self, trigger_value: Optional[float], threshold: Optional[float]
    ) -> str:
        """Format threshold comparison for human readability.
        ...
        try:
            ...
            if triggered_at:
                ...
                if duration_minutes > 60:  # Alert active for over 1 hour
                    ...
                    indicators["factors"].append(
                        "Alert has been active for over 1 hour"
                    )

                if duration_minutes > 240:  # Alert active for over 4 hours
                    indicators["urgency_level"] = "critical"
                    indicators["factors"].append(
                        "Alert has been active for over 4 hours"
                    )
            ...
            if trigger_value is not None and threshold is not None and threshold != 0:
                ...
                if breach_percentage > 200:  # More than 200% breach
                    indicators["urgency_level"] = "critical"
                    indicators["factors"].append(
                        f"Threshold breached by {breach_percentage:.0f}%"
                    )
                elif breach_percentage > 100:  # More than 100% breach
                    if indicators["urgency_level"] == "low":
                        indicators["urgency_level"] = "medium"
                    indicators["factors"].append(
                        f"Significant threshold breach ({breach_percentage:.0f}%)"
                    )
    ...
    def _calculate_alert_statistics(
        ...
        for alert in alerts:
            ...
            # Count urgency
            urgency = alert.get("urgency_indicators", {}).get(
                "urgency_level", "unknown"
            )
        ...
        average_age_minutes = (
            total_duration_minutes / total_alerts if total_alerts > 0 else 0
        )
        ...
        return {
            ...
            "stale_alerts": len(
                [a for a in alerts if a.get("duration", {}).get("is_stale", False)]
            ),
            "critical_alerts": severity_counts.get("Critical", 0),
        }

    def _generate_alert_summary(
        self, alerts: List[Dict[str, Any]], stats: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if critical_count > 0:
            recommendations.append(
                f"Immediately investigate {critical_count} critical alert{'s' if critical_count != 1 else ''}"
            )

        if stale_count > 0:
            recommendations.append(
                f"Review {stale_count} stale alert{'s' if stale_count != 1 else ''} - consider escalation or threshold adjustment"
            )

        if total_alerts > 10:
            recommendations.append(
                "High alert volume detected - consider reviewing monitor thresholds"
            )
    ...
    def _assess_business_impact(
        self, severity: Optional[str], monitor_name: str, trigger_value: Optional[float]
    ) -> str:
            ...
            actions.append(
                {
                    ...
                    "alert_ids": [
                        a["alert_id"] for a in escalation_alerts[:3]
                    ],  # Top 3
                    "urgency": "medium",
                }
            )
    ...
    async def enable_monitor(self, monitor_id: str) -> Dict[str, Any]:
        ...
        async def _enable_monitor_impl():
            # Validate monitor_id with enhanced validation
            validated_monitor_id = await validate_monitor_id(
                monitor_id, "enable_monitor"
            )

            # Get current monitor status before enabling
            try:
                current_monitor = await self.api_client.get_monitor(
                    validated_monitor_id
                )
                current_status = (
                    "disabled"
                    if current_monitor.get("isDisabled", False)
                    else "enabled"
                )
            ...
            try:
                enable_result = await self.api_client.enable_monitor(
                    validated_monitor_id
                )
            except RateLimitError as e:
                raise RateLimitError(
            ...
            try:
                updated_monitor = await self.api_client.get_monitor(
                    validated_monitor_id
                )
                new_status = (
                    "disabled"
                    if updated_monitor.get("isDisabled", False)
                    else "enabled"
                )
            except Exception as e:
                logger.warning(
        ...
        logger.info("Enabling monitor", extra={"monitor_id": monitor_id})
        ...
        try:
            ...
            response = {
                ...
                "status_change": {
                    ...
                    "new_status": "enabled",
                    "changed": True,
                    "change_timestamp": api_response.get(
                        "timestamp", datetime.now().isoformat()
                    ),
                },
                "timestamp": datetime.now().isoformat(),
            ...
            if e.status_code == 403:
                raise APIError(
                    ...
                    status_code=e.status_code,
                    request_id=e.request_id,
                    context={"operation": "enable_monitor", "monitor_id": monitor_id},
                ) from e
    ...
    async def disable_monitor(self, monitor_id: str) -> Dict[str, Any]:
        ...
        async def _disable_monitor_impl():
            ...
            try:
                disable_result = await self.api_client.disable_monitor(
                    validated_monitor_id
                )
            except RateLimitError as e:
                raise RateLimitError(
        ...
        logger.info("Disabling monitor", extra={"monitor_id": monitor_id})
        ...
        try:
            ...
            if e.status_code == 403:
                raise APIError(
                    ...
                    status_code=e.status_code,
                    request_id=e.request_id,
                    context={"operation": "disable_monitor", "monitor_id": monitor_id},
                ) from e
            elif e.status_code == 404:
                # Already handled above, but include for completeness
                raise
    ...
    async def validate_monitor_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async def _validate_monitor_config_impl():
            ...
            if query:
                try:
                    query_validation = await self.api_client.validate_monitor_query(
                        query=query, monitor_type=monitor_type
                    )

                    validation_result["query_syntax_valid"] = query_validation.get(
                        "valid", False
                    )
                    ...
                    if not query_validation.get("valid", False):
                        validation_result["valid"] = False
                        error_msg = query_validation.get(
            ...
            if trigger_conditions:
                try:
                    trigger_validation = (
                        await self._validate_trigger_conditions_comprehensive(
                            trigger_conditions
                        )
                    )

                    validation_result["trigger_conditions_valid"] = (
                        trigger_validation.get("valid", False)
                    )
                    validation_result["validation_details"][
                        "trigger_validation"
                    ] = trigger_validation
                ...
                validation_result["errors"].append(
                    "Trigger conditions are required but not provided"
                )
                validation_result["valid"] = False
            ...
            if notifications:
                try:
                    notification_validation = (
                        await self._validate_notification_configurations(notifications)
                    )

                    validation_result["notifications_valid"] = (
                        notification_validation.get("valid", False)
                    )
                    validation_result["validation_details"][
                        "notification_validation"
                    ] = notification_validation
            ...
            validation_result["summary"] = {
                ...
                "config_ready_for_creation": validation_result["valid"]
        ...
        try:
            ...
            trigger_conditions = config.get("trigger_conditions", {})
            trigger_validation_result = self._validate_trigger_conditions(
                trigger_conditions
            )

            validation_result["trigger_conditions_valid"] = trigger_validation_result[
                "valid"
            ]
            validation_result["validation_details"][
                "trigger_validation"
            ...
            validation_result["warnings"].extend(
                trigger_validation_result.get("warnings", [])
            )

            # Step 4: Validate notification configurations
            logger.debug("Validating notification configurations")
            notifications = config.get("notifications", [])
            notification_validation_result = self._validate_notification_configurations(
                notifications
            )

            validation_result["notifications_valid"] = notification_validation_result[
                "valid"
            ]
            validation_result["validation_details"][
                "notification_validation"
            ] = notification_validation_result

            if not notification_validation_result["valid"]:
                validation_result["valid"] = False
                validation_result["errors"].extend(
                    notification_validation_result["errors"]
                )

            validation_result["warnings"].extend(
                notification_validation_result.get("warnings", [])
            )
            ...
            validation_result["summary"] = {
                ...
                "config_structure_valid": (
                    validation_result["validation_details"]["config_structure"]["valid"]
                    if validation_result["validation_details"]["config_structure"]
                    else False
                ),
                "query_syntax_valid": validation_result["query_syntax_valid"],
                "trigger_conditions_valid": validation_result[
                    "trigger_conditions_valid"
                ],
    ...
    def _validate_trigger_conditions(
        ...
        """
        result = {"valid": True, "errors": [], "warnings": [], "conditions_checked": []}
        ...
        for trigger_type, condition in trigger_conditions.items():
            ...
            if not isinstance(condition, dict):
                condition_result["valid"] = False
                condition_result["errors"].append(
                    "Trigger condition must be a dictionary"
                )
                result["valid"] = False
            else:
                ...
                if (
                    time_range
                    and time_range.startswith("-")
                    and int(time_range[1:-1]) > 60
                    and time_range.endswith("m")
                ):
                    condition_result["warnings"].append(
    ...
    def _validate_notification_configurations(
        self, notifications: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Validate notification configurations.
        ...
        if not notifications:
            result["warnings"].append(
                "No notifications configured - monitor will not send alerts"
            )
            return result
            ...
            if not isinstance(notification, dict):
                ...
                if action_type == NotificationType.EMAIL:
                    ...
                    if not recipients:
                        notification_result["valid"] = False
                        notification_result["errors"].append(
                            "Email notifications require recipients"
                        )
                        result["valid"] = False
                    else:
                        # Validate email addresses
                ...
                if subject and len(subject) > 255:
                    notification_result["warnings"].append(
                        "Subject is longer than 255 characters"
                    )

                message_body = notification.get("message_body")
                if message_body and len(message_body) > 2000:
                    notification_result["warnings"].append(
                        "Message body is longer than 2000 characters"
                    )
    ...
    def _perform_additional_validation_checks(
        self, config: Dict[str, Any], validation_result: Dict[str, Any]
    ) -> None:
    ...
    async def get_monitor_history(
        self, monitor_id: str, from_time: str, to_time: str, limit: int = 100
    ) -> Dict[str, Any]:
        ...
        async def _get_monitor_history_impl():
            # Validate monitor_id
            validated_monitor_id = await validate_monitor_id(
                monitor_id, "get_monitor_history"
            )
            ...
            if not from_time or not isinstance(from_time, str):
                raise MonitorValidationError(
                    ...
                    context=create_monitor_error_context(
                        "get_monitor_history", monitor_id=validated_monitor_id
                    ),
                )
            ...
            try:
                ...
                trigger_patterns = {"analysis_error": str(e)}
        ...
        try:
            ...
            api_response = await self.api_client.get_monitor_history(
                monitor_id=monitor_id, from_time=from_time, to_time=to_time, limit=limit
            )
            ...
            logger.info(
                ...
                extra={
                    ...
                    "triggers_found": len(
                        [h for h in formatted_history if h.get("triggered", False)]
                    ),
                },
            )
            ...
            raise APIError(
                ...
                context={"operation": "get_monitor_history", "monitor_id": monitor_id},
            ) from e
        ...
        for entry in history_data:
            try:
                ...
                formatted_entry["performance_indicators"] = {
                    "execution_fast": (
                        entry.get("executionDurationMs", 0) < 5000
                    ),  # < 5 seconds
                    "data_volume_normal": (
                        entry.get("dataScannedBytes", 0) < 100 * 1024 * 1024
                    ),  # < 100MB
                    "result_count_reasonable": (
                        entry.get("resultCount", 0) < 10000
                    ),  # < 10k results
                }
                ...
                logger.warning(
                    f"Failed to format history entry: {str(e)}", extra={"entry": entry}
                )
    ...
    def _calculate_performance_metrics(
            ...
            metrics.update(
                {
                    ...
                    "avg_data_scanned_mb": round(
                        (total_data / len(data_scanned)) / (1024 * 1024), 2
                    ),
                }
            )
        else:
        ...
        metrics["performance_indicators"] = {
            "fast_execution_rate": (
                len([t for t in execution_times if t < 5000])
                / len(execution_times)
                * 100
                if execution_times
                else 0
            ),
            "slow_execution_count": len(
                [t for t in execution_times if t > 30000]
            ),  # > 30 seconds
            "high_data_volume_count": len(
                [d for d in data_scanned if d > 100 * 1024 * 1024]
            ),  # > 100MB
            "large_result_set_count": len(
                [r for r in result_counts if r > 10000]
            ),  # > 10k results
        }
    ...
    def _analyze_trigger_patterns(
        ...
        for entry in history_data:
            if entry.get("triggered", False):
                ...
                triggers_by_severity[severity] = (
                    triggers_by_severity.get(severity, 0) + 1
                )
        ...
        trigger_rate = (
            (total_triggers / total_executions * 100) if total_executions > 0 else 0.0
        )
        ...
        analysis = {
            ...
            "analysis_summary": {
                ...
                "severity_distribution": {
                    severity: (
                        round(count / total_triggers * 100, 1)
                        if total_triggers > 0
                        else 0
                    )
    ...
    def _calculate_trigger_frequency(
        ...
        triggers_per_hour = (
            len(parsed_timestamps) / total_hours if total_hours > 0 else 0
        )
        ...
        avg_triggers_per_hour = (
            len(parsed_timestamps) / 24 if len(parsed_timestamps) > 24 else 0
        )
        peak_hours = [
            hour for hour, count in hour_counts.items() if count > avg_triggers_per_hour
        ]
        ...
        return {
            ...
            "trigger_intervals": {
                "avg_interval_minutes": (
                    round(sum(intervals) / len(intervals), 2) if intervals else 0
                ),
    ...
    def _analyze_recent_trigger_trend(self, history_data: List[Dict[str, Any]]) -> str:
        ...
        # Sort by timestamp (most recent first)
        sorted_data = sorted(
            history_data, key=lambda x: x.get("timestamp", ""), reverse=True
        )
        ...
        recent_triggers = sum(
            1 for entry in recent_half if entry.get("triggered", False)
        )
    ...
    def _analyze_trigger_durations(
        self, triggers: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def handle_monitor_operation_error(
        ...
        """
        context = create_monitor_error_context(
            operation, monitor_id=monitor_id, monitor_name=monitor_name
        )
    ...
    async def validate_monitor_operation_preconditions(
        self, operation: str, monitor_id: Optional[str] = None, **operation_params
    ) -> Dict[str, Any]:
    ...
    def _generate_health_recommendations(
        self, health_status: str, health_issues: List[str]
    ) -> List[str]:
        ...
        if health_status == "unhealthy":
            recommendations.append(
                "Check Sumo Logic API connectivity and authentication"
            )
            recommendations.append("Verify network connectivity and firewall settings")
            recommendations.append(
                "Consider increasing timeout values if operations are timing out"
            )

        elif health_status == "degraded":
            recommendations.append(
                "Monitor error patterns and consider adjusting retry policies"
            )
            recommendations.append(
                "Check for rate limiting and adjust request frequency if needed"
            )
        ...
        for issue in health_issues:
            if "circuit breaker" in issue.lower():
                recommendations.append(
                    "Wait for circuit breaker recovery or check underlying service health"
                )
            elif "error count" in issue.lower():
                recommendations.append(
                    "Investigate specific error patterns and root causes"
                )
        ...
        if not recommendations:
            recommendations.append(
                "Monitor operations are healthy - continue normal operations"
            )
    ...
    def configure_enhanced_logging(
        self, log_level: str = "INFO", enable_structured_logging: bool = True
    ):
    ...
    async def log_monitor_operation_success(
        ...
        logger.info(
            f"Monitor operation completed successfully: {operation}", extra=log_context
        )
        ...
        try:
            ...
            logger.info(
                ...
                extra={
                    ...
                    "total_error_types": error_stats["monitor_tools_errors"][
                        "total_error_types"
                    ],
                },
            )
    ...
    def _generate_operational_insights(
        self, error_stats: Dict[str, Any], health_status: Dict[str, Any]
    ) -> List[str]:
        ...
        if total_errors == 0:
            insights.append(
                "No errors detected in monitor operations - system is operating normally"
            )
        elif total_errors < 5:
            insights.append(
                f"Low error rate detected ({total_errors} total errors) - system is stable"
            )
        elif total_errors < 20:
            insights.append(
                f"Moderate error rate detected ({total_errors} total errors) - monitor for trends"
            )
        else:
            insights.append(
                f"High error rate detected ({total_errors} total errors) - investigation recommended"
            )

        # Analyze circuit breaker status
        circuit_status = (
            error_stats.get("resilience_status", {})
            .get("circuit_breaker", {})
            .get("state")
        )
        if circuit_status == "open":
            insights.append(
                "Circuit breaker is open - service may be experiencing persistent failures"
            )
        elif circuit_status == "half_open":
            insights.append(
                "Circuit breaker is in recovery mode - monitoring service recovery"
            )
        ...
        for error_type, count in error_patterns.items():
            if count > 3:
                insights.append(
                    f"Frequent {error_type} errors detected ({count} occurrences)"
                )
        ...
        if health == "unhealthy":
            insights.append(
                "Monitor tools health is unhealthy - immediate attention required"
            )
        elif health == "degraded":
            insights.append(
                "Monitor tools health is degraded - performance may be impacted"
            )
    ...
    def _generate_operational_recommendations(
        ...
        error_patterns = error_stats.get("monitor_tools_errors", {}).get(
            "error_patterns", {}
        )
        ...
        for error_type, count in error_patterns.items():
            if "rate_limit" in error_type.lower() and count > 2:
                ...
                recommendations.append(
                    "Consider increasing timeout values or optimizing query performance"
                )
            elif "validation" in error_type.lower() and count > 3:
                recommendations.append(
                    "Review monitor configuration validation rules and user guidance"
                )
            elif "permission" in error_type.lower() and count > 1:
                recommendations.append(
                    "Review user permissions and access control settings"
                )

        # Add general recommendations
        total_errors = error_stats.get("monitor_tools_errors", {}).get(
            "total_errors", 0
        )
        if total_errors > 10:
            recommendations.append(
                "Consider implementing additional monitoring and alerting for monitor tools"
            )
            recommendations.append(
                "Review error handling patterns and retry logic configuration"
            )
